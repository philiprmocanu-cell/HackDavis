...
/** Twilio `<Connect><Stream>` expects μ-law 8 kHz from ConvAI; mismatch usually yields immediate 31921. */
...
/** Logs agent TTS/ASR formats at startup (same diagnosis path as education voice). */
function scheduleMedicalAgentTelephonyFormatCheck(agentIdRaw) {
  const skip = String(process.env.MEDICAL_AGENT_TELEPHONY_CHECK_OFF || "").toLowerCase();
  ...
  void (async () => {
    try {
      ...
      if (ok) {
        console.log(`[medical-voice] ConvAI agent "${label}" — TTS ${ttsFmt}, ASR ${asrFmt} (Twilio-ready).`);
        return;
      }
      console.warn(
        `[medical-voice] ConvAI agent "${label}" — TTS output=${ttsFmt}, ASR input=${asrFmt}. For Twilio, set **both** to ${TWILIO_MEDIA_STREAM_AUDIO} (μ-law 8kHz) in ElevenLabs → Agent → Voice + Advanced. Mismatch often causes 31921 and sub-second “empty” calls.`,
      );
    } catch (err) {
      console.warn("[medical-voice] ConvAI agent telephony check failed:", err?.message || err);
    }
  })();
}
...
export function logMedicalVoiceConfig() {
  ...
  if (missing.length === 0 && mode === "conversation") {
    ...
    scheduleMedicalAgentTelephonyFormatCheck(process.env.ELEVENLABS_AGENT_ID);
  }
}
...
export function attachMedicalVoiceRoutes(app) {
  ...
  app.post(VOICEMAIL_STREAM_STATUS_PATH, twilioUrlEncoded, (req, res) => {
    ...
    if (ev === "stream-error" || (terr !== undefined && terr !== "")) {
      const codeRaw = b.StreamErrorCode ?? b.stream_error_code;
      const code = codeRaw != null ? String(codeRaw).trim() : "";
      const hint31921 =
        code === "31921"
          ? " Often: ConvAI agent TTS/ASR not both ulaw_8000 — see startup [medical-voice] ConvAI agent … line. ElevenLabs conversation logs; https://www.twilio.com/docs/api/errors/31921"
          : "";
      console.warn(
        `[medical-voice] stream-error (Twilio passes little detail).${hint31921} Full payload:`,
        JSON.stringify(b),
      );
    } else if (medicalVoiceDebug()) {
